...
Title: Validation Performed After State Updates in brokenExecution()

Severity: Low because no incorrect state can persist due to EVM transaction
atomicity, but unnecessary storage operations are executed before validation.
...
The brokenExecution() function updates state variables before validating
the user input.
...
If the require() statement fails, the EVM reverts the entire transaction
and restores the previous state.

Although this does not create a direct security vulnerability, it violates
the Checks-Effects-Interactions (CEI) pattern and results in unnecessary
gas consumption.
...
- Harder auditability
- Increased risk of future vulnerabilities if external calls are added
  before validation

No permanent state corruption is possible because all storage changes are
rolled back during revert.
...
Observe that all temporary state changes are removed.

Root Cause:

The function performs state modifications before completing input
validation.

The require() statement is placed after storage updates instead of before
them.

Recommendation:

Validate all conditions before modifying storage.
...
This follows the Checks -> Effects -> Interactions (CEI) pattern and
avoids unnecessary gas expenditure on reverting transactions.
*/